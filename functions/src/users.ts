...
const usersCollecttion = 'Usuarios';

/**
 * Read the user document from Firestore
 */
export const getUser = async (uid: string): Promise<any> => {
    return await db.collection(usersCollecttion).doc(uid).get().then(doc => doc.data());
};
...
export const updateUser = async (uid: string, data: Object): Promise<any> => {
    const user = await db.collection(usersCollecttion).doc(uid).set(data, { merge: true });
    return user;
}
